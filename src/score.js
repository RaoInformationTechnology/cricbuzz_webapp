...
class score extends Component{
	...
	componentDidMount(){
		...
		fetch("https://cricapi.com/api/fantasySummary?apikey=KxFEC7542kcdZoSrfyOH452AoL42&unique_id="+matchId)
		.then(res => res.json())
		.then(matchData =>{
	...
	render() {
		...
		// let displayTeam;
		// if(this.state.stateTeam.length){
		// 	displayTeam = this.state.stateTeam.map((team, teamIndex) => {
		// 		return(
		// 			<div key={teamIndex}>
		// 				<p>{team.name}</p>
		// 				<table>
